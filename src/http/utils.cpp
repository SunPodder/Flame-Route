...
void* handle_connection(struct thread_args* args){
    ...
    if(response->status > 400){
        Logger::Error(method + " - " + request->path + " " + std::to_string(response->status) + " " + getHTTPStatusFromCode(response->status));
    } else {
        Logger::Log(method + " - " + request->path + " " + std::to_string(response->status) + " " + getHTTPStatusFromCode(response->status));
    }
